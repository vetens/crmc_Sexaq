...
c...........check ids
          ...
          do i=1,nidcod(n)
            ...
            elseif(istptl(j).eq.0.and.idcod(i,n).eq.9995)then    !all particles but nuclei
              if(abs(idptl(j)).lt.10000
     $          .and.(idptl(j).lt.17.or.idptl(j).gt.19)) go=.true.     
            ...
            if(ivar(1,n).lt.100.or.ivar(2,n).lt.100)then
              ...
              if(ivar(1,n).lt.100)then
                ...
                if(x.ge.xmin(n).and.x.le.xmax(n))then
                  ...
                  elseif(norm3.eq.8.and.x.ne.0.)then
                    y=y*x*pptl(4,j)/abs(pptl(3,j))
                  endif
                  if(icnx(n))then
...
c     7:  y => y/x/(x-m)
c     8:  y => 2.*y*x/beta   (beta=abs(p3)/p4)
c
c  h  0: normal
c     1: accumulated
c
...
c laboratory momentum)
        x=xMdiff
      elseif(inom.eq.368)then  !weighted spectrum moment for pi+
        x=xmom(1)
      elseif(inom.eq.369)then  !weighted spectrum moment for pi-
        x=xmom(2)
      elseif(inom.eq.370)then  !weighted spectrum moment for K+
        x=xmom(3)
      elseif(inom.eq.371)then  !weighted spectrum moment for K-
        x=xmom(4)
      elseif (inom.eq.372) then !---------
        x=sigddex
...
c spectrum weighted moments for muon inclusive spectra
          if(iframe.eq.12)then
            xf=pptl(3,i)/elab   !x_f in lab frame only
            if(xf.gt.0.)then
              if(idptl(i).eq.120)xmom(1)=xmom(1)+xf**1.7
              if(idptl(i).eq.-120)xmom(2)=xmom(2)+xf**1.7
              if(idptl(i).eq.130)xmom(3)=xmom(3)+xf**1.7
              if(idptl(i).eq.-130)xmom(4)=xmom(4)+xf**1.7
            endif
          endif
